...
%maps = {"corridor", "home", "warehouse", "willowgarage"};
%maps = {"dirtrack"};
maps = {"csal"};
...
for m = 1:size(maps,2)
  ...
  % Load map
  map = maps(m){1};
  map_str = strcat('/home/li9i/catkin_ws/src/relief_fmt_global_localisation/experiments_logs/evaluation_scripts/maps/', map);
  ms = csv2cell(map_str);
  ...
  if strcmp(map, 'csal')
    ...
    h_4 = drawArrow (arrow_4, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_4.body; h_4.wing(:)], 'k',1,1);

    arrow_5 = [pose_5(1,1:2) tip_5];
    h_5 = drawArrow (arrow_5, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_5.body; h_5.wing(:)], 'k',1,1);

    arrow_6 = [pose_6(1,1:2) tip_6];
    h_6 = drawArrow (arrow_6, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_6.body; h_6.wing(:)], 'k',1,1);

    arrow_7 = [pose_7(1,1:2) tip_7];
    h_7 = drawArrow (arrow_7, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_7.body; h_7.wing(:)], 'k',1,1);

    arrow_8 = [pose_8(1,1:2) tip_8];
    h_8 = drawArrow (arrow_8, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_8.body; h_8.wing(:)], 'k',1,1);

    arrow_9 = [pose_9(1,1:2) tip_9];
    h_9 = drawArrow (arrow_9, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_9.body; h_9.wing(:)], 'k',1,1);

    arrow_10 = [pose_10(1,1:2) tip_10];
    h_10 = drawArrow (arrow_10, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_10.body; h_10.wing(:)], 'k',1,1);

    arrow_11 = [pose_11(1,1:2) tip_11];
    h_11 = drawArrow (arrow_11, 1, 0.5 , 0.1);
    arrayfun(@(x,y)set(x,'color',y), [h_11.body; h_11.wing(:)], 'k',1,1);
  ...
  if (store == 1)
    img_file = strcat('/media/ext/backup_files/AUTh/RELIEF/relief-fmt-global-localisation-paper/figures/maps/simulations/', map, '.eps');
