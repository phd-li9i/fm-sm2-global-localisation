...
using namespace ranges_rot;
using namespace benchmark_rot;
...
////////////////////////////////////////////////////////////////////////////////
	if (utils::has("BresenhamsLine", methods) || utils::has("bl", methods)) {
		...
		BenchmarkRot<BresenhamsLine> mark = BenchmarkRot<BresenhamsLine>(bl);
...
////////////////////////////////////////////////////////////////////////////////
	if (utils::has("RayMarching", methods) || utils::has("rm", methods)) {
		...
		std::chrono::duration<double> construction_dur =
			std::chrono::duration_cast<std::chrono::duration<double>>(construction_end - construction_start);
		BenchmarkRot<RayMarching> mark = BenchmarkRot<RayMarching>(rm);
...
////////////////////////////////////////////////////////////////////////////////
	if (utils::has("CDDTCast", methods) || utils::has("cddt", methods)
		...
		if (utils::has("CDDTCast", methods) || utils::has("cddt", methods)) {
			...
			BenchmarkRot<CDDTCast> mark = BenchmarkRot<CDDTCast>(rc);
			std::cout << "...Running grid benchmark" << std::endl;
			if (DO_LOG) {
				tlog.str("");
				mark.set_log(&tlog);
...
////////////////////////////////////////////////////////////////////////////////
	if (utils::has("GiantLUTCast", methods) || utils::has("glt", methods)) {
		...
		BenchmarkRot<GiantLUTCast> mark = BenchmarkRot<GiantLUTCast>(glt);
...
////////////////////////////////////////////////////////////////////////////////
	if (utils::has("RayMarchingGPU", methods) || utils::has("rmgpu", methods)) {
		...
		if (FLAGS_which_benchmark == "grid") {
			int num_samples = BenchmarkRot<RayMarching>::num_grid_samples(GRID_STEP, GRID_RAYS, GRID_SAMPLES, map.width, map.height);
			float *samples = new float[num_samples*3];
			float *outs = new float[num_samples];


			BenchmarkRot<RayMarching>::get_grid_samples(samples, GRID_STEP, GRID_RAYS, GRID_SAMPLES, map.width, map.height);
			// warmup
...
////////////////////////////////////////////////////////////////////////////////
		if (FLAGS_which_benchmark == "random") {
			...
			// warm up
			BenchmarkRot<RayMarching>::get_random_samples(samples, RANDOM_SAMPLES, map.width, map.height);
